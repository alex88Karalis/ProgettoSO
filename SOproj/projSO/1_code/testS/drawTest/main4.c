        ...
        if(pipeData.type=='T'){
        	...
        	switch(pipeData.id){
        		case 1:
        			...
        			if(pipeData.x!=old_pos[1].x || pipeData.y!=old_pos[1].y){
        				
        				pulisciTroncoInMatrice(old_pos[1].y,old_pos[1].x,screenMatrix);
        		
        				stampaTroncoInMatrice(pipeData.y,pipeData.x,screenMatrix);
        	
        				// aggiorno coords_old
        				old_pos[1].x=pipeData.x;
        				old_pos[1].y=pipeData.y;
        			}
        			break;
        		case 2:
        			// se pipedata è diverso da troncodata_old allora le coordinate sono cambiate pulisco la matrice e ristampo sulla matrice
        			if(pipeData.x!=old_pos[2].x || pipeData.y!=old_pos[2].y){
        				
        				pulisciTroncoInMatrice(old_pos[2].y,old_pos[2].x,screenMatrix);
        				
        				stampaTroncoInMatrice(pipeData.y,pipeData.x,screenMatrix);
        	
        				// aggiorno coords_old
        				old_pos[2].x=pipeData.x;
        				old_pos[2].y=pipeData.y;
        			}
        			break;
        		case 3:
        			// se pipedata è diverso da troncodata_old allora le coordinate sono cambiate pulisco la matrice e ristampo sulla matrice
        			if(pipeData.x!=old_pos[3].x || pipeData.y!=old_pos[3].y){
        	
		      			pulisciTroncoInMatrice(old_pos[3].y,old_pos[3].x,screenMatrix);
        				
        				stampaTroncoInMatrice(pipeData.y,pipeData.x,screenMatrix);
		      	
		      			// aggiorno coords_old
		      			old_pos[3].x=pipeData.x;
		      			old_pos[3].y=pipeData.y;
        			}
        			break;
        		default:
        			break;
