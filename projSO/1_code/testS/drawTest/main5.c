...
void aggiornaPosizioneOggetto(struct PipeData *pipeData,struct PipeData *old_pos,char (*screenMatrix)[WIDTH]);
void inizializzaMatriceSchermo(char (*screenMatrix)[WIDTH]);
    ...
    while (1) {
        ...
        if(pipeData.type=='X'){
        	...
        	if(pipeData.x!=old_pos[0].x || pipeData.y!=old_pos[0].y){
        		...
        		screenMatrix[old_pos[0].y][old_pos[0].x]=' ';
        ...
        if(pipeData.type=='T'){
        	switch(pipeData.id){
        		case 1:
        			aggiornaPosizioneOggetto(&pipeData, &old_pos[1], screenMatrix); // aggiorna pozione oggetto in matrice schermo
        			break;
        		case 2:
        			aggiornaPosizioneOggetto(&pipeData, &old_pos[2], screenMatrix); // aggiorna pozione oggetto in matrice schermo
        			break;
        		case 3:
        			aggiornaPosizioneOggetto(&pipeData, &old_pos[3], screenMatrix); // aggiorna pozione oggetto in matrice schermo
        			break;
        		default:
        			break;
...
void aggiornaPosizioneOggetto(struct PipeData *pipeData,struct PipeData *old_pos,char (*screenMatrix)[WIDTH]){
  ...
  if(pipeData->x!=old_pos->x || pipeData->y!=old_pos->y){
  	...
  	pulisciTroncoInMatrice(old_pos->y,old_pos->x,screenMatrix);
